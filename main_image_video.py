...
class ExpressionApp(QWidget):
    def __init__(self):
        ...
        self.original_label.setMinimumWidth(640)
        ...
        # --- 좌우 메인 영역 ---
        hbox = QHBoxLayout()
        hbox.addWidget(self.original_label, 1)
    ...
    def process_frame(self, image):
        ...
        self.original_label.setPixmap(pixmap.scaled(
            self.original_label.width(), self.original_label.height(), Qt.KeepAspectRatio, Qt.SmoothTransformation))
        ...
        self.update_results((label, prob, box, image.shape))
    ...
    def update_results(self, prediction):
        ...
        label, prob, box, shape = prediction
        for emo, val in zip(self.exp_dict.values(), prob):
            p = round(val * 100, 2)
            self.progress_bars[emo].setValue(int(p))
            self.percentage_labels[emo].setText(f"{p:.2f}%")
        ...
        self.result_label.setText(f"<b>Prediction: <span style='color:#ff9500'>{label}</span> ({prob[pred_idx]*100:.2f}%)</b>")

        result_json = {
            "predicted_label": label,
            "predicted_index": pred_idx,
            "softmax": {k: round(float(v), 4) for k, v in zip(self.exp_dict.values(), prob)},
